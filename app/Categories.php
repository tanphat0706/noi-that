...
class Categories extends Model
{
    ...
    public function productOfCateForMenu($id){
        $products = Products::leftJoin('categories','products.category_id', '=' , 'categories.id')
            ...
            ->select('products.alias','products.id','products.name','products.image_1','products.price','products.category_id','products.highlight')->get();
        return $products;
    }
    public function productOfCate($id){
        $products = Products::leftJoin('categories','products.category_id', '=' , 'categories.id')
            ->where('products.category_id',$id)
            ->select('products.alias','products.id','products.name','products.image_1','products.price','products.category_id','products.highlight')->paginate(10);
        return $products;
    }
